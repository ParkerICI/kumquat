...
citrus.plotModelDifferentialFeatures.classification = function(differentialFeatures,features,modelOutputDirectory,labels,...){
  for (cvPoint in names(differentialFeatures)){
    ...
    melted = reshape2::melt(data.frame(features[,nonzeroFeatureNames,drop=F],labels=labels,check.names=F),id.vars="labels")
    ...
    p <- (ggplot2::ggplot(melted, ggplot2::aes(x=factor(labels), y=value))
      ...
      + ggplot2::geom_boxplot(outlier.colour=rgb(0,0,0,0),colour=rgb(0,0,0,.3)) 
...
citrus.overlapDensityPlot = function(clusterDataList,backgroundData){
  ...
  p <- (ggplot2::ggplot(data=combined, ggplot2::aes(x=value, y = ..scaled..,fill=src)) 
    ...
    + ggplot2::theme_bw()
    + ggplot2::scale_fill_manual(values = c("Background" = rgb(.3,.3,1,.2), "Cluster" = rgb(1,.3,.3,.5)))
    + ggplot2::theme(legend.position="bottom",axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.title=element_blank())
...
#' @author Robert Bruggner
#' 
plot.citrus.regressionResult = function(citrus.regressionResult,outputDirectory,citrus.foldClustering,citrus.foldFeatureSet,citrus.combinedFCSSet,plotTypes=c("errorRate","stratifyingFeatures","stratifyingClusters","clusterGraph"),...){
addtlArgs = list(...)
